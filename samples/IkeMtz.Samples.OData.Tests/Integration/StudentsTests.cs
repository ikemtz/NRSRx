...
namespace IkeMtz.Samples.OData.Tests.Integration
{
  ...
  public partial class StudentsTests : BaseUnigrationTests
  {
    ...
    public async Task GetGroupByStudentsTest()
    {
      ...
      try
      {
        resp = await client.GetAsync($"odata/v1/{nameof(Student)}s?$apply=groupby(({nameof(Student.FirstName)},{nameof(Student.BirthDate)}))");
      }
      catch (Exception) { }
    ...
    public async Task GetStudentsWithExpansionsTest()
    {
      ...
      student.StudentCourses.Add(Factories.StudentCourseFactory(student, course, school));
      using var srv = new TestServer(TestHostBuilder<Startup, IntegrationODataTestStartup>()
          .ConfigureTestServices(x =>
          {
            ExecuteOnContext<DatabaseContext>(x, db =>
